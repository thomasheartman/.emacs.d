...
*** Removing ~suspend-frame~
    ...
    #+begin_src emacs-lisp
      (define-key (current-global-map) [remap suspend-frame]
        (lambda ()
          (interactive)
...
** notmuch (email)
   To be able to send email with notmuch, we need to make sure that Emacs is
   configured to use the application.
   #+begin_src emacs-lisp
     (setq message-send-mail-function 'message-send-mail-with-sendmail)
   #+end_src
   Furthermore, we want sent mail to not be tagged with 'inbox' and 'unread', but rather with 'sent'.
   #+begin_src emacs-lisp
